...
with(hpc_subdata,{
  # create subplot 1
  plot(hpc_subdata$Time,hpc_subdata$Global_active_power,type="l",  xlab="",ylab="Global Active Power")  
  # create subplot 2
  plot(hpc_subdata$Time,hpc_subdata$Voltage, type="l",xlab="DateTime",ylab="Voltage")
  # create subplot 3
  plot(hpc_subdata$Time,hpc_subdata$Sub_metering_1,type="n",xlab="",ylab="Energy sub metering")
  with(hpc_subdata,lines(Time, Sub_metering_1, col="black"))
  with(hpc_subdata,lines(Time, Sub_metering_2, col="red"))
  with(hpc_subdata,lines(Time, Sub_metering_3, col="blue"))
  legend("topright", lty=1, col = c("black","red","blue"), legend = c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),cex=0.7)
  # create subplot 4
  plot(hpc_subdata$Time,hpc_subdata$Global_reactive_power, type="l", xlab="DateTime", ylab="Global_reactive_power")
})
# close output device
dev.off()
